...
#else
static void gfxDrawTextScreen(uint8_t g_vram[0x20000], uint16_t, uint16_t, uint16_t, uint32_t *,
		const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette);
#endif
static void gfxDrawRotScreen(uint8_t g_vram[0x20000], uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     ...
			     uint32_t*,
			     const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette);
static void gfxDrawRotScreen16Bit(uint8_t g_vram[0x20000], uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
				  int&, int&,
				  uint32_t*,
				  uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC);
static void gfxDrawRotScreen256(uint8_t g_vram[0x20000], uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
				int&, int&,
				uint32_t*,
				const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT, const uint16_t *palette);
static void gfxDrawRotScreen16Bit160(uint8_t g_vram[0x20000], uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
				     int&, int&,
				     uint32_t*,
				     uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT);
static void gfxDrawSprites(GBALCD &lcd, uint32_t *, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT);
...
inline constexpr int g_coeff[32]{
...
static inline void gfxDrawTextScreen(uint8_t g_vram[0x20000], uint16_t control, uint16_t hofs, uint16_t vofs,
				     uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette)
{
	const size_t charBankBaseOffset = ((control >> 2) & 0x03) * 0x4000;
  const uint16_t *screenBase = (uint16_t*)&g_vram[((control >> 8) & 0x1f) * 0x800];
  uint32_t prio = ((control & 3) << 25) + 0x1000000;

  static constexpr int widthMap[4]{256, 512, 256, 512};
  static constexpr int heightMap[4]{256, 256, 512, 512};
  uint32_t mapSize = (control >> 14) & 3;
  ...
  int yshift = ((yyy >> 3) << 5);
  if ((control)&0x80) {
    const uint16_t* screenSource = screenBase + 0x400 * (xxx >> 8) + ((xxx & 255) >> 3) + yshift;
    for (int x = 0; x < 240; x++) {
...
#else
static inline void gfxDrawRotScreen(uint8_t g_vram[0x20000], uint16_t control,
				    uint16_t x_l, uint16_t x_h,
				    uint16_t y_l, uint16_t y_h,
				    uint16_t pa,  uint16_t pb,
				    uint16_t pc,  uint16_t pd,
				    ...
				    const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette)
{
  ...
  static constexpr int rotMap[4]{128, 256, 512, 1024};
  int sizeX = rotMap[(control >> 14) & 3];
  int sizeY = sizeX;
...
static inline void gfxDrawRotScreen16Bit(uint8_t g_vram[0x20000], uint16_t control,
					 uint16_t x_l, uint16_t x_h,
					 uint16_t y_l, uint16_t y_h,
					 uint16_t pa,  uint16_t pb,
					 uint16_t pc,  uint16_t pd,
...
{
	(void)x_l; (void)x_h; (void)y_l; (void)y_h; // unused
  const uint16_t *screenBase = (uint16_t*)&g_vram[0];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 240;
  int sizeY = 160;
...
static inline void gfxDrawRotScreen256(uint8_t g_vram[0x20000], uint16_t control,
				       uint16_t x_l, uint16_t x_h,
				       uint16_t y_l, uint16_t y_h,
				       uint16_t pa,  uint16_t pb,
				       uint16_t pc,  uint16_t pd,
				       ...
				       const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT, const uint16_t *palette)
{
  const uint8_t *screenBase = (DISPCNT & 0x0010) ? &g_vram[0xA000] : &g_vram[0x0000];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 240;
  int sizeY = 160;
...
static inline void gfxDrawRotScreen16Bit160(uint8_t g_vram[0x20000], uint16_t control,
					    uint16_t x_l, uint16_t x_h,
					    uint16_t y_l, uint16_t y_h,
					    uint16_t pa,  uint16_t pb,
					    uint16_t pc,  uint16_t pd,
					    ...
					    uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT)
{
  const uint16_t *screenBase = (DISPCNT & 0x0010) ? (uint16_t*)&g_vram[0xa000] : (uint16_t*)&g_vram[0];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 160;
  int sizeY = 128;
...
static inline void gfxDrawSprites(GBALCD &lcd, uint32_t* lineOBJ, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT)
{
	...
	auto& lineOBJpixleft = lcd.lineOBJpixleft;
  ...
  if (coreOptions.layerEnable & 0x1000) {
    const uint16_t *sprites = (uint16_t*)g_oam;
    const uint16_t *spritePalette = &((uint16_t*)g_paletteRAM)[256];
      ...
      if (a0 & 0x0100) {
        ...
        if ((t >= 0) && (t < fieldY)) {
          ...
          if ((sx < 240) || startpix) {
            ...
            // int t2 = t - (fieldY >> 1);
            int rot = (a1 >> 9) & 0x1F;
            const uint16_t* OAM = (uint16_t*)g_oam;
            int dx = READ16LE(&OAM[3 + (rot << 4)]);
            if (dx & 0x8000)
              dx |= 0xFFFF8000;
            int dmx = READ16LE(&OAM[7 + (rot << 4)]);
