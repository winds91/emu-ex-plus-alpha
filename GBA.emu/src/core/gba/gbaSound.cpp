...
#define remake_stereo_buffer() remake_stereo_buffer(gba)
...
#define g_ioMem gba.mem.ioMem.b
	...
	if (which_timer == timer && enabled) {
		...
		if (count <= 16) {
				...
				if (count <= 16) {
						...
						for (int n = 4; n--;) {
							soundEvent16(gba, io_reg,     0);
							soundEvent16(gba, io_reg + 2, 0);
						}
				}
		}
	}
...
#define apply_control() apply_control(gba)
...
void soundEvent8(GBASys &gba, uint32_t address, uint8_t data)
{
...
static void apply_volume(GBASys &gba, bool apu_only = false)
{
	//if ( gb_apu )
	{
		...
		gb_apu.volume(gbApuSoundVolume_ * apu_vols [g_ioMem[SGCNT0_H] & 3]);
	}
...
#define apply_volume(...) apply_volume(gba, ## __VA_ARGS__)
...
#define write_SGCNT0_H(data) write_SGCNT0_H(gba, data)

void soundEvent16(GBASys &gba, uint32_t address, uint16_t data)
{
	switch (address) {
	case SGCNT0_H:
		write_SGCNT0_H(data);
	...
	default:
		soundEvent8(gba, address & ~1, (uint8_t) (data     )); // even
		soundEvent8(gba, address |  1, (uint8_t) (data >> 8)); // odd
		break;
	}
}
...
#define apply_muting() apply_muting(gba)
...
void soundReset(GBASys &gba)
{
	...
	soundEvent8(gba,  NR52, (uint8_t) 0x80);
}
...
static constexpr variable_desc old_gba_state[] = {
...
static constexpr variable_desc old_gba_state2[] = {
...
// New state format
static constexpr variable_desc gba_state[] = {
	// PCM
	LOAD(int, pcm[0].readIndex),
	LOAD(int, pcm[0].count),
	LOAD(int, pcm[0].writeIndex),
