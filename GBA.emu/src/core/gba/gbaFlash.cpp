...
void flashDetectSaveType(const uint8_t *g_rom, const int size) {
...
void flashWrite(uint32_t address, uint8_t byte)
{
    ...
    case FLASH_CMD_5:
        if (byte == 0x30) {
            ...
            // CHIP ERASE
            memset(flashSaveMemory.data(), 0xff, g_flashSize);
            systemSaveUpdateCounter = SYSTEM_SAVE_UPDATED;
            flashReadState = FLASH_ERASE_COMPLETE;
        } else {
            flashState = FLASH_READ_ARRAY;
...
void flashSaveGame(uint8_t*& data)
{
    ...
    utilWriteDataMem(data, flashSaveData3(flashSaveMemoryTemp, g_flashSize).data());
}

void flashReadGame(const uint8_t*& data)
{
  uint8_t flashSaveMemoryTemp[SIZE_FLASH1M]{};
  int flashSizeTemp = g_flashSize;
  utilReadDataMem(data, flashSaveData3(flashSaveMemoryTemp, flashSizeTemp).data());
  if(flashSizeTemp != g_flashSize)
  {
    EmuEx::GbaSystem::log.warn("expected flash size:{} but got {} from state", g_flashSize, flashSizeTemp);
  }
...
void flashSaveGame(gzFile gzFile)
{
    uint8_t flashSaveMemoryTemp[SIZE_FLASH1M]{};
    IG::copy_n(flashSaveMemory.data(), flashSaveMemory.size(), flashSaveMemoryTemp);
    utilWriteData(gzFile, flashSaveData3(flashSaveMemoryTemp, g_flashSize).data());
}
