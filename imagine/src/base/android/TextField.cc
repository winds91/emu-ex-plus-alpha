...
void AndroidTextField::setupTextEntryJni(JNIEnv* env, jobject textEntry)
{
	if(jFinishTextInput) [[likely]]
		return;
	log.info("setting up text input JNI");
			...
			(void*)+[](JNIEnv* env, jobject, jlong nUserData, jstring jStr, jboolean processText, [[maybe_unused]] jboolean isDoingDismiss)
			{
				...
				if(delegate)
				{
					if(jStr)
					{
						...
						log.info("running text entry callback with text:{}", str);
						delegate(str);
						env->ReleaseStringUTFChars(jStr, str);
					}
					else
					{
						log.info("canceled text entry callback");
						delegate(nullptr);
					}
				}
				else
				{
					log.info("text entry has no callback");
				}
			}
		}
	};
...
void TextField::place(WindowRect rect)
{
	if(!jTextEntry)
		return;
	textRect = rect;
	log.info("placing text edit box at {},{} with size {},{}", rect.x, rect.y, rect.xSize(), rect.ySize());
