...
{
	logMsg("Z80 bank unused read %06X (%x)", address, Z80.pc.d);
  return (address & 1) ? 0x00 : 0xFF;
}

void zbank_unused_w(unsigned int address, unsigned int data)
{
	logMsg("Z80 bank unused write %06X = %02X", address, data);
}

unsigned int zbank_lockup_r(unsigned int address)
{
	logMsg("Z80 bank lockup read %06X (%x)", address, Z80.pc.d);
  if (!config_force_dtack)
  {
    Z80.cycleCount = 0xFFFFFFFF;
    zstate = 0;
  }
  return 0xFF;
}
...
{
  logMsg("Z80 bank lockup write %06X = %02X (%x)", address, data, Z80.pc.d);
  if (!config_force_dtack)
  {
    Z80.cycleCount = 0xFFFFFFFF;
    zstate = 0;
...
void zbank_write_ctrl_io(unsigned int address, unsigned int data)
{
  switch ((address >> 8) & 0xFF)
  {
    ...
    case 0x11:  /* BUSREQ */
    {
      if (!(address & 1))
      {
        gen_zbusreq_w(data & 1, Z80.cycleCount);
        return;
      }
      zbank_unused_w(address, data);
      return;
    ...
    case 0x12:  /* RESET */
    {
      if (!(address & 1))
      {
        gen_zreset_w(data & 1, Z80.cycleCount);
        return;
      }
      zbank_unused_w(address, data);
      return;
...
unsigned int zbank_read_vdp(unsigned int address)
{
  switch (address & 0xFD)
  {
    ...
    case 0x04:    /* CTRL */
    {
      return (((vdp_68k_ctrl_r(Z80.cycleCount) >> 8) & 3) | 0xFC);
    }

    case 0x05:    /* CTRL */
    {
      return (vdp_68k_ctrl_r(Z80.cycleCount) & 0xFF);
    }
      
    case 0x08:    /* HVC */
    case 0x0C:
    {
      return (vdp_hvc_r(Z80.cycleCount) >> 8);
    }

    case 0x09:    /* HVC */
    case 0x0D:
    {
      return (vdp_hvc_r(Z80.cycleCount) & 0xFF);
    }
