...
CDAccess_Image::CDAccess_Image(VirtualFS* vfs, const std::string& path, bool image_memcache) : NumTracks(0), FirstTrack(0), LastTrack(0), total_sectors(0)
{
 try
 {
  if(IG::endsWithAnyCaseless(path, ".bin", ".iso"))
  {
   ImageOpenBinary(vfs, path, IG::endsWithAnyCaseless(path, ".iso"));
  }
  else
...
int CDAccess_Image::Read_Raw_Sector(uint8 *buf, int32 lba)
{
  uint8 SimuQ[0xC];
  int32 track;
  ...
  else
  {
   if(ct->AReader)
   {
    ...
    {
     MDFN_printf("Error: frames_read out of range: %llu\n", (unsigned long long)frames_read);
     frames_read = 0;
    }
   ...
   else	// Binary, woo.
   {
	...
	case DI_FORMAT_MODE1:
		ct->fp->readAtPos(buf + 12 + 3 + 1, 2048, SeekPos);
		SeekPos += 2048;
		encode_mode1_sector(lba + 150, buf);
		break;
	...
	case DI_FORMAT_MODE2:
		ct->fp->readAtPos(buf + 16, 2336, SeekPos);
		SeekPos += 2336;
	...
	// about cdrdao...).
	case DI_FORMAT_MODE2_FORM1:
		ct->fp->readAtPos(buf + 24, 2048, SeekPos);
		SeekPos += 2048;
		//encode_mode2_form1_sector(lba + 150, buf);
		break;

	case DI_FORMAT_MODE2_FORM2:
		ct->fp->readAtPos(buf + 24, 2324, SeekPos);
		SeekPos += 2324;
		//encode_mode2_form2_sector(lba + 150, buf);
		break;
    ...
    if(ct->SubchannelMode)
			ct->fp->readAtPos(buf + 2352, 96, SeekPos);
	 }
